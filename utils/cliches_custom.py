...
def main():
    ...
    cliches_path = Path("../data/raw/cliches_custom.csv")
    out_path = Path("../data/processed/cliche_hits_custom.csv")

    df_t = pd.read_csv(transcripts_path)
    df_c = pd.read_csv(cliches_path)

    if "cliche" not in df_c.columns:
        raise ValueError("cliches_custom.csv must contain a 'cliche' column.")

    cliche_list = df_c["cliche"].dropna().astype(str).tolist()
    cliche_patterns = build_cliche_patterns(cliche_list)

    # Filter for Manchester United this season only
    # Assumes 'club' and 'season' columns exist in transcripts.csv
    current_season = df_t["season"].max() if "season" in df_t.columns else None
    # df_t_filtered = df_t[(df_t["club"] == "Manchester United") & (df_t["season"] == current_season)]
    df_t_filtered = df_t[(df_t["manager"] == "Ange Postecoglou") & (df_t["season"] == current_season)]
    out_rows = []
